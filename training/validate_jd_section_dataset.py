"""Validate the JD section-labeling dataset (datasets/jd_section_splitting/vX/labeled_lines.jsonl).

Mirrors training/validate_section_dataset.py (the CV version) for the JD
section-classifier distillation dataset produced by
scripts/generate_jd_labeling_data.py.

Usage:
    python training/validate_jd_section_dataset.py
    python training/validate_jd_section_dataset.py --input datasets/jd_section_splitting/v0.1/labeled_lines.jsonl
"""
...
class DatasetValidator:
    ...
    def validate(self, input_path: Path) -> int:
        ...
        jd_ids = self.validate_ids(records)

        for record in records:
            self.validate_jd(record)

        if not records:
            self.error(f"No labeled job descriptions found in {input_path}")
    ...
    def validate_ids(self, records: list[JsonlRecord]) -> set[str]:
        ...
        for record in records:
            ...
            if not isinstance(record_id, str) or not record_id.strip():
                self.error(f"{self.location(record)} JD has missing or invalid id")
                continue
        ...
        for duplicate_id in sorted(duplicate_ids):
            self.error(f"Duplicate JD id found: {duplicate_id}")

        return seen

    def validate_jd(self, record: JsonlRecord) -> None:
        data = record.data
        missing = JD_REQUIRED_FIELDS - set(data)
        if missing:
        ...
        if len(lines) < MIN_LINES_PER_JD:
            self.warning(
                f"{self.location(record)} JD {data.get('id')} has only {len(lines)} lines "
                f"(recommended minimum {MIN_LINES_PER_JD})"
            )

        labels_seen: set[str] = set()
        for i, item in enumerate(lines, 1):
            if not isinstance(item, dict):
                self.error(f"{self.location(record)} JD {data.get('id')} line {i} must be an object")
                continue

            missing_line_fields = LINE_REQUIRED_FIELDS - set(item)
            if missing_line_fields:
                self.error(
                    f"{self.location(record)} JD {data.get('id')} line {i} "
            ...
            if not isinstance(text, str) or not text.strip():
                self.error(f"{self.location(record)} JD {data.get('id')} line {i} 'text' must be a non-empty string")
        ...
        if len(labels_seen) < MIN_DISTINCT_LABELS:
            self.warning(
                f"{self.location(record)} JD {data.get('id')} uses only {len(labels_seen)} "
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate the JD section-labeling dataset.")
...
def main() -> int:
    ...
    if validator.label_counts and not args.quiet:
        ...
        print(f"JDs: {validator.jd_count}   Labeled lines: {validator.line_count}")
