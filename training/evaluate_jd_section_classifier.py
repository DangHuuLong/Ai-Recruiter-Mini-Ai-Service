"""
Evaluate the trained JD section-classifier (classifier head + Viterbi
smoothing) on a held-out test split. Mirrors
training/evaluate_section_classifier.py (the CV version).

Usage:
  python -m training.evaluate_jd_section_classifier
  python -m training.evaluate_jd_section_classifier \\
      --test datasets/jd_section_splitting/v0.1/classifier/jd_section_classifier_test.jsonl \\
      --model models/jd-section-classifier-v0.1
"""
...
DEFAULT_TEST = Path("datasets/jd_section_splitting/v0.1/classifier/jd_section_classifier_test.jsonl")
DEFAULT_MODEL_DIR = Path("models/jd-section-classifier-v0.1")
...
def _group_by_doc(rows: list[dict]) -> dict[str, list[dict]]:
    by_doc: dict[str, list[dict]] = {}
    for r in rows:
        by_doc.setdefault(r["doc_id"], []).append(r)
    for doc_rows in by_doc.values():
        doc_rows.sort(key=lambda r: r["line_index"])
    return by_doc
...
def evaluate(test_path: Path, model_dir: Path) -> None:
    ...
    by_doc = _group_by_doc(test_rows)
    ...
    row_index_of: dict[tuple[str, int], int] = {
        (r["doc_id"], r["line_index"]): i for i, r in enumerate(test_rows)
    }

    for doc_id, doc_rows in by_doc.items():
        idxs = [row_index_of[(doc_id, r["line_index"])] for r in doc_rows]
        doc_emissions = emission_log_probs[idxs]
        true_labels = [r["label"] for r in doc_rows]

        no_smoothing_labels = [LABELS[i] for i in np.argmax(doc_emissions, axis=1)]
        smoothed_indices = viterbi_decode(doc_emissions, transition_log_probs)
    ...
    print(f"JDs evaluated: {len(by_doc)}   Total lines: {len(test_rows)}")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate the JD section-classifier on a held-out test split.")
