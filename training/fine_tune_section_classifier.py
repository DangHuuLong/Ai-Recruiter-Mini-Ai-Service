"""
Train the section-classifier student model: frozen sentence embeddings +
structural features -> logistic-regression head, plus a Viterbi transition
matrix estimated from training label sequences.

Unlike training/fine_tune_cross_encoder_*.ipynb (always run on Colab/Kaggle
GPU — see docs/fine-tuning-colab-guide.md), this trains entirely on local
CPU: the only transformer forward pass is a single batched embedding call
over each split's lines, not iterative fine-tuning of the transformer
itself. Fitting the classifier head on frozen embeddings is a minutes-scale
job even on a laptop CPU.

Usage:
  python -m training.fine_tune_section_classifier
  python -m training.fine_tune_section_classifier \\
      --train datasets/section_splitting/v0.1/classifier/section_classifier_train.jsonl \\
      --validation datasets/section_splitting/v0.1/classifier/section_classifier_validation.jsonl \\
      --output models/section-classifier-v0.1
"""
...
DEFAULT_TRAIN = Path("datasets/section_splitting/v0.1/classifier/section_classifier_train.jsonl")
DEFAULT_VALIDATION = Path("datasets/section_splitting/v0.1/classifier/section_classifier_validation.jsonl")
DEFAULT_OUTPUT = Path("models/section-classifier-v0.1")
...
def estimate_transition_matrix(rows: list[dict], alpha: float = TRANSITION_SMOOTHING_ALPHA) -> np.ndarray:
    ...
    label sequences (grouped per resume, ordered by line_index), with
    additive smoothing so every transition has non-zero probability."""
    K = len(LABELS)
    counts = np.full((K, K), alpha, dtype=np.float64)

    by_resume: dict[str, list[tuple[int, str]]] = {}
    for r in rows:
        by_resume.setdefault(r["resume_id"], []).append((r["line_index"], r["label"]))

    for seq in by_resume.values():
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train the section-classifier student model.")
