...
Resume parsing is document-based: the Backend sends a short-lived signed URL for the uploaded CV, and the AI service downloads the file, extracts text, normalizes it, parses it, and returns both `raw_text` and `parsed_data`.
...
Parses an uploaded resume document into raw text and structured resume data.
...
- `file_type` — Supported values: `PDF`, `DOCX`.
- `signed_url` — Short-lived URL generated by the Backend for the AI service to download the file.
- `checksum` — Optional checksum from the stored file asset.
...
- `text_extraction_method` currently supports `PDF_TEXT` and `DOCX_TEXT`.
- If a PDF has no selectable text, the AI service returns a warning. OCR fallback is intentionally not implemented yet.
...
JD parsing remains text-based because the product currently stores JD content from form input rather than uploaded documents.
