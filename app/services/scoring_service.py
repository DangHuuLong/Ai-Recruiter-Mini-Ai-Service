...
from app.scorers.cv_jd_scorer import build_summary, score_application_mock
...
class ScoringService:
    def score_application(self, request: ScoreApplicationRequest) -> EvaluationResult:
        ...
        try:
            ...
            # summary embeds the score as text ("Overall score: X.") — must be regenerated
            # against the blended score, otherwise it keeps quoting the pre-blend
            # rule-only score while overall_score (and the UI's headline number) shows the
            # blended one. Found via a real user report: badge showed 65, summary text
            # still said "Overall score: 53.13" underneath it.
            matched = [skill.skill_name for skill in rule_result.skills if skill.type == "MATCHED"]
            missing = [skill.skill_name for skill in rule_result.skills if skill.type == "MISSING"]
            blended_summary = build_summary(blended, matched, missing)
            return rule_result.model_copy(
                update={"overall_score": blended, "similarity_score": ml_score, "summary": blended_summary}
            )
