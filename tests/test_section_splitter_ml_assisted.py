...
class TestMlAssistedSplitReconciliation:
    def test_reclassifies_only_the_other_bucket(self) -> None:
        """A no-header preamble ('Experienced backend engineer.') falls into
        regex's 'other' bucket. The rest of the document has real headers,
        so those lines must stay exactly as the regex assigned them."""
        text = (
    ...
    def test_lines_the_model_still_calls_other_stay_in_other(self) -> None:
        text = "Some genuinely unclassifiable separator line\nSKILLS\nPython"
        ...
        assert result["other"] == "Some genuinely unclassifiable separator line"
    ...
    def test_reclassified_lines_prepend_existing_section_content(self) -> None:
        ...
        target section (e.g. an inline-style second summary line)."""
        text = (
